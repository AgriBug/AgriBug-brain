...
IMG_SIZE = 50
LR = 1e-3
MODEL_NAME = 'agribugplantdiseasedetection-{}-{}.model'.format(LR, '2conv-basic')
...
def analysis(filepath):
    verify_data = proccess_verify_data(filepath)
    ...
    convent = conv_2d(convnet, 128, 3, activation='relu')
    convnet = max_pool_2d(convnet, 3)

    convnet = conv_2d(convnet, 32, 3, activation='relu')
    convnet = max_pool_2d(convnet, 3)

    convnet = conv_2d(convent, 64, 3, activation='relu')
    convnet = max_pool_2d(convnet, 3)
